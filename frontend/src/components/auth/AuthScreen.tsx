...
import { ArrowLeft, LockKeyhole, ShieldCheck, TrendingUp } from "lucide-react";
...
const providerButtons = [
  { label: "Apple", Icon: AppleIcon, iconClassName: "providerApple" },
  { label: "Google", Icon: GoogleIcon, iconClassName: "providerGoogle" },
  { label: "Meta", Icon: MetaIcon, iconClassName: "providerMeta" },
];
      ...
      <div className={styles.frame}>
        <section className={styles.formPanel}>
          <div className={styles.formPanelInner}>
            ...
            {error ? <InlineMessage message={error} className={styles.error} id={errorId} /> : null}
            ...
            <div className={styles.providers}>
              {providerButtons.map((provider) => (
                <button key={provider.label} type="button" className={styles.providerButton}>
                  <span className={styles.providerGlyph} aria-hidden="true">
                    <provider.Icon className={`${styles.providerIcon} ${styles[provider.iconClassName]}`} />
                  </span>
                  <span className={styles.srOnly}>
                    {providerAction} with {provider.label}
                  </span>
                </button>
              ))}
            </div>
          ...
          <div className={styles.visualSurface}>
            <div className={styles.visualTopBar}>
              ...
              <div className={styles.visualStatus}>
                <ShieldCheck size={14} />
                Secure account surface
              </div>
            </div>

            <div className={styles.accessGrid}>
              {metrics.map((metric) => (
                <div key={metric.label} className={styles.accessCard}>
            ...
            <div className={styles.previewCard}>
              ...
              <div className={styles.previewHighlights}>
                {highlights.map((highlight) => (
                  <div key={highlight.title} className={styles.previewItem}>
